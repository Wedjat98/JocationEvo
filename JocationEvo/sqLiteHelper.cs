    ...
    public class SqLiteHelper
    {
        ...
        public void RemovePasswordProtection()
        {
            try
            {
                ...
                // Execute the command to remove the password
                using (var command = new SQLiteCommand("PRAGMA rekey = '';", dbConnection))
                {
                    command.ExecuteNonQuery();
                }

                // Update the connection string to not use a password for subsequent connections
                dbConnectionstr.Password = "";
        ...
        public SqLiteHelper(string connectionString)
        {
            try
            {
                ...
                dbConnectionstr.Password = "";      //设置密码，SQLite ADO.NET实现了数据库密码保护 
                dbConnection.ConnectionString = dbConnectionstr.ToString();
                dbConnection.Open();
            }
            catch (Exception e)
